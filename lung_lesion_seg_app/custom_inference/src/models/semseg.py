...
class SemSeg(torch.nn.Module):
    ...
    def vis_on_batch(self, batch, savedir_image, save_preds=False):
        ...
        vis = label2rgb(pred_mask_vis[:, :, 0].numpy(), image=None, colors=colors, bg_label=255, bg_color=None, alpha=0.6, kind='overlay')
        
        
        vis = mark_boundaries(
            vis, pred_mask_vis[:, :, 0].numpy().astype('uint8'), color=(1, 1, 1))
        # img = cv2.imread(vis,3)
        cv2.imwrite('g.png',vis/255.0)
        ...
        if save_preds:
            ...
            for u in range(self.n_classes):
                ...
                # pred = np.array(pred_numpy==u).astype('uint8')
                # print(pred)
                # cv2.imshow('l',pred)
                # cv2.waitKey(0)
                # pred.show()
